...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2) {
	...
	// ll1_size + 2 를 하는 이유는 병합 과정에서 ll1리스트가 ll2와 교차하면서 노드가 추가될 때,
	//  여유 공간이 필요하기 때문이다 여유 공간이 없다면 isnertNode와 removeNode 시 의 여유공간이 없기에 
	// 특히 마지막 i 순회시 값을 제대로 넣거나 뺄수 없게 된다 . 
    for(int i = 0; i <= ll1_size + 2 || ll2_current != NULL; i++)  {
        if (i & 1) {
            insertNode(ll1, i, ll2_current->item);
