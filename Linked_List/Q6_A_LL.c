...
int moveMaxToFront(ListNode **ptrHead) {
    if (*ptrHead == NULL || (*ptrHead)->next == NULL) {
        return -1;  // 리스트가 비어 있거나 노드가 하나만 있는 경우
    }

    ListNode *current = *ptrHead;
    ListNode *maxNode = *ptrHead;  // 최대값을 가진 노드를 추적
    ListNode *prevMaxNode = NULL;  // 최대값 노드의 이전 노드를 추적
    ...
    // 최대값이 이미 첫 번째 노드인 경우 아무것도 하지 않음
    if (maxNode == *ptrHead) {
        return 0;  // 이미 맨 앞에 있음
    }
    ...
    // 최대값을 리스트의 맨 앞으로 이동
    if (prevMaxNode != NULL) {
        prevMaxNode->next = maxNode->next;  // 이전 노드의 next를 갱신
    }
    maxNode->next = *ptrHead;  // 최대값 노드를 맨 앞에 연결
    *ptrHead = maxNode;        // 헤드를 최대값 노드로 변경

    return 1;  // 성공적으로 이동 완료
}
