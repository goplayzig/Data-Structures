...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList) {
    ...
    int firstSize = (ll->size + 2 / 2) / 2;
    int ll_size = ll->size;
    for (int i = 0; i <= ll_size && ll_current != NULL; i++) {
        next_node = ll_current->next;   
        if (i < firstSize) {
            insertNode(resultFrontList, i, ll_current->item);
        } else {
            insertNode(resultBackList, resultBackList->size, ll_current->item);
        }
        ll_current = next_node;
    }
}
