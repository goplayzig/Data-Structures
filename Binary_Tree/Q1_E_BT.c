...
int identical(BTNode *tree1, BTNode *tree2) {
    ...
    if (tree1 == NULL && tree2 == NULL) { // 두가지 트리 모두 비어 있을시 예외처리 
        return 1;
    }

    if (tree1 == NULL || tree2 == NULL) { // 둘중 하나의 트리만 비어있을때 
        return 0;
    }

    if (tree1->item != tree2->item) { //트리 의 값이 일치 하지 않을때 
        return 0;
    }
    ...
    // 왼쪽 오른쪽 나눠 재귀 호출 이때
    //  만약 한부분이 라도 0 이라면 AND 연산자를 통해 최종적으로 0 을 반환 
